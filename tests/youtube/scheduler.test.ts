...
describe('planTranslationWindows', () => {
  test('plans current window and lookahead when buffer low', () => {
    expect(planTranslationWindows({ ...emptyState, ccEnabled: true, currentTimeMs: 12_000, translatedUpToMs: 15_000 })).toEqual([
  ...
  test('bounds window by duration', () => {
    expect(planTranslationWindows({ ...emptyState, ccEnabled: true, currentTimeMs: 50_000, translatedUpToMs: 50_000, durationMs: 65_000 })).toEqual([
