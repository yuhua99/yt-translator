import type { CaptionMode, CaptionSegment, CaptionTrack, CapturedCaptionResponse, ParsedCaptions } from './caption-types';
...
function createTrack(url: string): CaptionTrack {
  ...
  const videoId = parsedUrl.searchParams.get('v') ?? parsedUrl.searchParams.get('video_id') ?? 'unknown-video';
...
function parseXmlCaptions(responseText: string, track: CaptionTrack): CaptionSegment[] {
  ...
  for (const match of responseText.matchAll(textTagPattern)) {
    ...
    segments.push(createSegment(track, segments.length, startMs, durationMs === undefined ? undefined : startMs + durationMs, text));
  }
...
function createSegment(track: CaptionTrack, index: number, startMs: number, endMs: number | undefined, text: string): CaptionSegment {
