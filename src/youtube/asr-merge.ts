...
function initialSplit(events: AsrEvent[]): AsrEvent[][] {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (
      nextEvent &&
      BREAK_WORDS.has((currentEvent.text + nextEvent.text).trim().toLowerCase()) &&
      elapsed > BREAK_MIN_TIME_MS
    ) {
...
function createMergedSegment(
  group: readonly AsrEvent[],
  nextGroup: readonly AsrEvent[] | undefined,
  index: number,
): CaptionSegment {
  const first = group[0]
